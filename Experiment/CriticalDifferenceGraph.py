...
def plot_cd_graph(variables_list, names_list, n_runs, alpha="0.05", width=6, textspace=1, filename=None, reverse=True):
    assert len(variables_list) == len(names_list)
    for i in range(len(variables_list)):
        mean = np.mean(variables_list[i])
        std = np.std(variables_list[i])
        names_list[i] += "({:.2f}%, {:.2f}%)".format(mean, std)
        
    avranks = averaged_ranks(*variables_list, reverse=reverse, average=True)
    print("Average ranks: ", avranks)
    cd = compute_CD(avranks, n_runs, alpha=alpha, test='nemenyi') #tested on n datasets
    print("CD: ", cd)
    graph_ranks(avranks, names_list, cd=cd, width=width, textspace=textspace, filename=filename)
    plt.show()    
