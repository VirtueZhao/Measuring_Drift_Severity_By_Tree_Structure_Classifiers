...
def generate_data(concept_size, drift_type):
    ...
    print("Generating AGRAWAL - " + concept_size +'(' + str(example_num) + ')' + "-" + drift_type + '(' + str(drift_width) + ')')
    data_generator = ConceptDriftStream(
        ...
        drift_stream=(ConceptDriftStream(
            ...
            drift_stream=AGRAWALGenerator(classification_function=4, random_state=3, balance_classes=True,
                                          perturbation=0),
    ...
    DD_file = "Case_Study/Case_Study_Results/AGRAWAL_" + concept_size + "_" + drift_type + "_DD.csv"
    TDM_file = "Case_Study/Case_Study_Results/AGRAWAL_" + concept_size + "_" + drift_type + "_TDM.csv"
